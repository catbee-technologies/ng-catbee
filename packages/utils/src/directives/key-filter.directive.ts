...
export class KeyFilter implements Validator {
  ...
  private isNavKeyPress(e: KeyboardEvent): boolean {
    let k = e.keyCode;
    k = this.platformService.browser['safari'] ? SAFARI_KEY_MAP[k] || k : k;
  ...
  private isSpecialKey(e: KeyboardEvent): boolean {
    ...
    return (
      ...
      k == 17 ||
      (k >= 18 && k <= 20) ||
      (!!this.platformService.browser['opera'] &&
        !e.shiftKey &&
  ...
  private getKey(e: KeyboardEvent): number {
    const k = e.keyCode || e.charCode;
    return this.platformService.browser['safari'] ? SAFARI_KEY_MAP[k] || k : k;
  }
  ...
  onKeyPress(e: KeyboardEvent): void {
    ...
    const browser = this.platformService.browser;
