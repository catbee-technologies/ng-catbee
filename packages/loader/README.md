...
> A modern, customizable Angular library for displaying beautiful loading indicators and spinners with 50+ animation styles — fully compatible with Server-Side Rendering (SSR) and featuring advanced options like fullscreen mode, custom templates, scroll blocking, and multiple concurrent loaders.
...
- 🔒 **Scroll Blocking**: Prevent scrolling while loader is active
...
```bash
npm install @ng-catbee/loader --save
```
...
@import '@ng-catbee/loader/css/ball-spin-clockwise.css';

/* Or import multiple animations */
@import '@ng-catbee/loader/css/ball-grid-pulse.css';
@import '@ng-catbee/loader/css/pacman.css';
@import '@ng-catbee/loader/css/line-scale.css';
```
...
**Note:** Replace `ball-spin-clockwise.css` with the animation name you want to use. The CSS filename matches the animation name (e.g., `ball-grid-pulse` → `ball-grid-pulse.css`).
...
export class DelayedLoaderComponent {
  ...
  async performOperation() {
    // Show loader after 300ms delay (prevents flash for quick operations)
    await this.loaderService.show('default', {}, 300);
    
    await this.quickOperation();
    
    // Hide immediately
    await this.loaderService.hide('default');
  }

  private async quickOperation() {
    // If this completes in < 300ms, loader never shows
  }
}
```
...
export const appConfig: ApplicationConfig = {
  providers: [
    provideCatbeeLoader({
      defaultAnimation: 'ball-spin-clockwise',
      defaultSize: 'medium',
      defaultOverlayColor: 'rgba(0, 0, 0, 0.8)',
      defaultLoaderColor: '#ffffff',
...
export class ServiceDemoComponent {
  ...
  async demonstrateAPI() {
    ...
    this.loaderService.observe('default').subscribe(state => {
...
| `blockScroll` | `boolean` | `true` | Block page scrolling |
...
|--------|------------|---------|-------------|
| `show()` | `name: string, options?: LoaderDisplayOptions, delay?: number` | `Promise<void>` | Show a loader |
...
| `observe()` | `name: string` | `Observable<LoaderState>` | Observe loader state changes |
...
The loader works seamlessly with Server-Side Rendering:

```typescript
// No special configuration needed!
// The service automatically handles SSR context
await loaderService.show('default'); // Works on both server and client
```
...
- [Installation](https://catbee.npm.hprasath.com/docs/@ng-catbee/loader/installation)
- [Basic Usage](https://catbee.npm.hprasath.com/docs/@ng-catbee/loader/usage/basic)
- [API Reference](https://catbee.npm.hprasath.com/docs/@ng-catbee/loader/api-reference)
- [Type Definitions](https://catbee.npm.hprasath.com/docs/@ng-catbee/loader/types)
