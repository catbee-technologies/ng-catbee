...
describe('CatbeeLoaderService', () => {
  ...
  describe('show', () => {
    ...
    it('should emit state change when showing loader', async () => {
      const statePromise = firstValueFrom(service.observe('test-loader'));
    ...
    it('should show loader with blockScroll option', async () => {
      await service.show('scroll-loader', { blockScroll: true });

      const state = service.getState('scroll-loader');
      expect(state?.blockScroll).toBe(true);
    });
    ...
    it('should log showing loader', async () => {
      await service.show('log-loader', { message: 'Test' });

      expect(consoleLogSpy).toHaveBeenCalledWith(
        'Showing loader:',
        'log-loader',
        jasmine.objectContaining({ message: 'Test' })
      );
    });
  });
  ...
  describe('hide', () => {
    ...
    it('should emit state change when hiding loader', async () => {
      const statePromise = firstValueFrom(service.observe('test-loader').pipe(take(1)));
  ...
  describe('observe', () => {
    it('should return observable for specific loader', done => {
      service.observe('observable-loader').subscribe(state => {
    ...
    it('should only emit states for specified loader', done => {
      ...
      service.observe('specific-loader').subscribe(() => {
        emissionCount++;
      });
    ...
    it('should observe loader with null name as default', done => {
      service.observe(null).subscribe(state => {
  ...
  describe('edge cases', () => {
    ...
    it('should handle all display options at once', async () => {
      ...
      expect(state?.blockScroll).toBe(options.blockScroll);
    });
  });
});
