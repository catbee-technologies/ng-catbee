...
describe('CatbeeLoader', () => {
  ...
  describe('initialization', () => {
    ...
    it('should subscribe to loader service on init', () => {
      spyOn(service, 'observe').and.callThrough();

      component.ngOnInit();

      expect(service.observe).toHaveBeenCalledWith('default');
    });
  });
  ...
  describe('service overrides', () => {
    ...
    it('should handle all service override options', async () => {
      const options = {
        ...
        loaderColor: '#0000ff',
        size: 'small' as LoaderSize,
        animation: 'ball-beat' as LoaderAnimation,
        fullscreen: false,
        zIndex: 5000,
  ...
  describe('scroll blocking', () => {
    ...
    beforeEach(() => {
      document = TestBed.inject(DOCUMENT) as Document;
      document.body.style.overflow = '';
    });

    // Skip this test as effects don't fire reliably in the test environment
    xit('should block scroll when loader is visible and blockScroll is true', async () => {
      // blockScroll defaults to true
      await service.show('default');
      fixture.detectChanges();
      ...
      expect(document.body.style.overflow).toBe('hidden !important');
    });

    xit('should not block scroll when blockScroll is false', async () => {
    ...
    xit('should restore scroll when loader is hidden', async () => {
