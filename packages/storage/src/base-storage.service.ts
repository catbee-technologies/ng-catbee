...
export abstract class BaseStorageService {
  ...
  get(key: string, skipDecoding: boolean = false): string | null {
    ...
    try {
      ...
      return value !== null
        ? this.encoder.decode(value, this.getStorageType(), skipDecoding, this.getStorageName())
        : null;
    } catch (error) {
   ...
   * @param key - The storage key.
   * @param skipDecoding - If true, skip decoding
   * @returns The parsed number, or `null` if not found or invalid.
   */
  getNumber(key: string, skipDecoding: boolean = false): number | null {
   ...
   * @param key - The storage key.
   * @param skipDecoding - If true, skip decoding
   * @returns The parsed boolean value, or `null` if not found or invalid.
   */
   ...
   * @param skipDecoding - If true, skip decoding
   * @returns The stored enum value if valid, otherwise `null`.
   */
  getEnum<T extends string>(key: string, enumValues: readonly T[], skipDecoding: boolean = false): T | null {
    const value = this.get(key, skipDecoding);
  ...
  /**
   * Retrieves a string value from a storage with validation and auto-set default.
   *
   * @param key - The storage key.
  ...
  /**
   * Retrieves a boolean value from a storage with validation and auto-set default.
   *
   * Recognizes common boolean string representations:
   * - Truthy: 'true', '1', 'yes', 'on'
   * - Falsy: 'false', '0', 'no', 'off'
  ...
  /**
   * Retrieves a numeric value from a storage with validation and auto-set default.
   *
   * @param key - The storage key.
  ...
  /**
   * Retrieves an enum value from a storage with validation and auto-set default.
   *
   * @template T - The enum type (must extend string).
   * @param key - The storage key.
  ...
  /**
   * Retrieves and parses a JSON value from storage with auto-set default.
   *
   * @template T - The expected type of the parsed object.
   * @param key - The storage key.
  ...
  /**
   * Retrieves and parses an array from storage with auto-set default.
   *
   * @template T - The expected type of array elements.
   * @param key - The storage key.
   ...
   */
  updateJson<T extends Record<string, unknown>>(key: string, updates: Partial<T>, defaultValue: T | null): void {
    const current = this.getJson<T>(key);
    this.setJson(key, { ...(defaultValue ?? {}), ...(current ?? {}), ...updates });
  }
