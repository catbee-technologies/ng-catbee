...
|--------|-------------|---------|
| `decodePayload<T>(token: string)` | Decode JWT payload with type safety | `T | null` |
| `decode<T>(token: string)` | Decode complete JWT (header, payload, signature) | `DecodedJwt<T> | null` |
| `isExpired(token: string, offsetSeconds?: number)` | Check if token is expired | `boolean` |
| `isValidFormat(token: string)` | Validate JWT format | `boolean` |
| `getExpirationDate(token: string)` | Get expiration as Date object | `Date | null` |
| `getIssuedDate(token: string)` | Get issued-at as Date object | `Date | null` |
| `getRemainingTime(token: string)` | Get remaining seconds until expiration | `number | null` |
| `watchExpiry(token: string, tickMs: number)` | Observe remaining time until expiration | `Observable<number>` |
| `getClaim<T>(token: string, claim: string)` | Extract specific claim with type safety | `T | null` |
